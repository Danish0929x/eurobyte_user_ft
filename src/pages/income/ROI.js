...
function ROI() {
  ...
  const fetchTransactions = async () => {
    setIsLoading(true);
    try {
      const transactions = await getTransactions("ROI", "credited"); // Example: Fetch transactions with remark 'Growth Level' and type 'credited'
      ...
      {isLoading ? (
        ...
        <div className="dashboard">
          <div className="team-in">
            <h1 className="main-heading">ROI Income</h1>
            <br />
            
            {/* Transactions Table */}
            <CustomTable data={data} columns={columns} heading="ROI Income" />
          </div>
        </div>
      )}
    </>
