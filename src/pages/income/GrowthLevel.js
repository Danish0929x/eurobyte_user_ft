...
function GrowthLevel() {
  ...
  const fetchTransactions = async () => {
    setIsLoading(true);
    try {
      const transactions = await getTransactions("Growth Level", "credited"); // Example: Fetch transactions with remark 'Growth Level' and type 'credited'
      ...
      {isLoading ? (
        ...
        <div className="dashboard">
          <div className="team-in">
            <h1 className="main-heading">Growth Level Bonus</h1>
            <br />
            
            {/* Transactions Table */}
            <CustomTable data={data} columns={columns} heading="Growth Level Bonus" />
          </div>
        </div>
      )}
    </>
